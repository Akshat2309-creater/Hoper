...
const Crisis = () => {
  ...
  return (
    <section id="crisis" className="bg-off-white py-12 px-4 sm:px-6 sm:py-16 md:py-20">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-10 lg:gap-12 items-stretch">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            {statistics.map((stat, index) => (
              <div key={index} className="group cursor-pointer rounded-xl border border-border bg-card px-4 py-6 text-center text-card-foreground shadow-sm transition-all duration-300 hover:scale-[1.02] hover:border-deep-purple/35 hover:bg-deep-purple hover:shadow-md active:scale-95 sm:px-4 sm:py-8">
                <div className="flex flex-col items-center space-y-2">
                  ...
                  <p className="text-foreground text-xs sm:text-sm group-hover:text-white text-center leading-tight px-2">{stat.description}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6 md:gap-8 mt-10 sm:mt-12 md:mt-16">
          {factors.map((factor, index) => (
            <div key={index} className="group cursor-pointer rounded-xl border border-red-600/80 bg-card p-5 text-card-foreground shadow-sm transition-all duration-300 hover:scale-[1.02] hover:bg-red-600 hover:shadow-md active:scale-95 sm:p-6">
              <h3 className="text-lg sm:text-xl font-bold text-red-600 mb-3 sm:mb-4 group-hover:text-white">{factor.title}</h3>
              <p className="text-sm sm:text-base text-muted-foreground leading-relaxed group-hover:text-white">{factor.description}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6 md:gap-8 mt-8 sm:mt-10">
          {thoughtLeaders.map((item, index) => (
            <div
              key={index}
              className="border-2 border-black/40 bg-card dark:border-white/15 rounded-lg shadow-xl overflow-hidden transition-all duration-300 transform hover:-translate-y-1 hover:shadow-2xl flex flex-col h-full"
            >
              <div className="relative pt-6 pb-2 flex items-center justify-center h-[360px] sm:h-[380px]">
                <div className="w-[95%] h-[92%] rounded-xl bg-muted/50 shadow-lg flex items-center justify-center dark:bg-muted/30">
              ...
              <div className="px-4 sm:px-5 pb-5 pt-3 space-y-3 flex-1 flex flex-col">
                <div className="rounded-2xl border border-red-100/80 bg-muted/40 p-4 shadow-sm flex-1 flex flex-col dark:border-red-900/40 dark:bg-muted/25">
